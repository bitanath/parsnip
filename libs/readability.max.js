  ...
  Readability.prototype = {
    ...
    _grabArticle(page) {
      ...
      while (true) {
        ...
        //   node.dataset.coordinates = JSON.stringify({x,y})
        ...
        this._forEachNode(elementsToScore, function (elementToScore) {
          ...
          if (innerText.length < 25) {
            if(innerText.length > 1){
                // elementToScore.dataset.querySelector = getUniqueSelector(elementToScore)
            }
            return;
          }
          ...
          this._forEachNode(ancestors, function (ancestor, level) {
            ...
            // ancestor.readability.querySelector = getUniqueSelector(ancestor)
            // ancestor.dataset.querySelector = getUniqueSelector(ancestor)
    ...
    annotateDOMWithSelectors(rootElement) {
        ...
        // Process all elements using a TreeWalker
        ...
        while (node = walker.nextNode()) {
            ...
            node.dataset.cssSelector = selector;
        }
    },   
    findFirstDescendantWithMultipleChildren(root) {
        // Use a TreeWalker to efficiently traverse the DOM
        const walker = document.createTreeWalker(
            root,
            NodeFilter.SHOW_ELEMENT,
            {
                acceptNode: function(node) {
                    // Check if the node has more than one child element
    ...
    normalizeLineBreaks(text) {
        return text
            ...
            .replace(/(\r\n)+/g, '\r\n')
            // Then handle old Mac line endings
            .replace(/\r+/g, '\r')
            // Finally handle Unix line endings
            .replace(/\n+/g, '\n');
    },
    ...
    parse() {
      ...
      this.annotateDOMWithSelectors(this._doc)
      ...
      Array.from(this.findFirstDescendantWithMultipleChildren(articleContent.querySelector("#readability-page-1")).children).forEach((node,index)=>{
        nodeArray.push({
            tagName: node.tagName.toLowerCase(),
            querySelector: node.dataset.cssSelector,
            text: this.normalizeLineBreaks(node.innerText),
            index
        })
      })
